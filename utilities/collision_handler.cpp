...
void collision_handler::run() {
    ...
    error_check("collision_handler::update_objects");
}

void collision_handler::ship_against_wall() {
    if (!o_register->ready()) return;
    auto ship = o_register->ship;
    auto arena = o_register->arena;

    // update_objects arena wall material if spaceship is close
...
//    if (ship->position.x - ship->radius <= -arena->width || ship->position.x + ship->radius >= arena->width ||
//        ship->position.y - ship->radius <= -arena->height || ship->position.y + ship->radius >= arena->height ||
//        ship->position.z - ship->radius <= -arena->length || ship->position.z + ship->radius >= arena->length)
//        manager->end_game();
}
...
//    auto ship = o_register->ship;
//
//    for (int i = 0; i < o_register->asteroids.size(); ++i) {
//        auto a = o_register->asteroids.at(i);
...
//    if (!o_register->ready()) return;
//
//    if (!o_register->bullets.empty()) {
//        for (int i = o_register->bullets.size() - 1; i >= 0; --i) {
//            bullet* b = o_register->bullets.at(i);
//
...
//    if (!o_register->bullets.empty() && !o_register->asteroids.empty()) {
//        for (int i = o_register->bullets.size() - 1; i >= 0; --i) {
//            bullet* b = o_register->bullets.at(i);
//            for (int j = o_register->asteroids.size() - 1; j >= 0; --j) {
//                asteroid* a = o_register->asteroids.at(j);
//
...
//
//                    if (++a->hit_count == a->hp) {
//                        if (a->can_split) o_register->split_asteroid(j);
...
//                    vector3d n = (a1->position - a2->position).get_normalised();
//
//                    double a1n = a1->direction.dot(n);
//                    double a2n = a2->direction.dot(n);
//
//                    double p = ((a1n - a2n) * 2.0) / (a1->radius + a2->radius);
//                    a1->direction -= (n * p * a2->radius);
//                    a2->direction -= (n * p * a1->radius);
